...
class JobScraperAgent:
    def __init__(self):
        ...
        self.setup_driver()
        self.last_run = None
        self.run_interval = 3600  # Default: 1 hour
                        ...
                        try:
                            ...
                            if post_date >= cutoff_date:
                                ...
                                if cursor.rowcount > 0:
                                    ...
                                    print(f"New job found: {title}, Posted: {post_date.strftime('%Y-%m-%d')}")
    ...
    def send_email(self, job_results):
        if not job_results:
            return
        # New connection for email updates
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
    ...
    def suggest_terms(self):
        if self.term_frequency:
            print("Search term frequency:", dict(self.term_frequency))
            most_common = self.term_frequency.most_common(1)[0][0]
            print(f"Suggested focus term: {most_common}")
    ...
    def interactive_loop(self):
        while True:
            ...
            if command == "run":
                ...
                try:
                    new_interval = int(command.split()[1]) * 3600
                ...
                new_term = command[4:].strip()
                if new_term not in search_terms:
                    search_terms.append(new_term)
                    print(f"Added '{new_term}' to search terms.")
            elif command == "quit":
                break
            else:
                print("Commands: run, interval <hours>, suggest, add <term>, quit")
